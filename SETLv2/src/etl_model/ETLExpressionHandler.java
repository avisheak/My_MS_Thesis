...
public class ETLExpressionHandler implements ETLOperation {
	...
	private String outputMap = "", conceptString = "";
			...
			public void run() {
				try {
					...
					if (conceptString.isEmpty()) {
						result += expressionHandler.transformOnLiteral(getMappingFile(), getSourceABoxFile(), getResultFile(), outputMap);
					} else {
						result += expressionHandler.transformOnLiteral(getMappingFile(), getSourceABoxFile(), getResultFile(), outputMap, conceptString);
					}
	...
	public boolean getInput(JPanel panel, HashMap<String, LinkedHashSet<String>> inputParamsMap) {
		...
		JComboBox<Object> comboBoxSourceABox = new JComboBox<Object>();
		...
		LinkedHashSet<String> sourceHashSet = inputParamsMap.get(SPARQL_FILE);
		...
		btnOpenAboxFile.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (!(filePath == null)) {
					...
					if (!filePath.equals("")) {
						DefaultComboBoxModel<Object> comboBoxModel = (DefaultComboBoxModel<Object>) comboBoxSourceABox.getModel();
		...
		JComboBox<?> comboBoxTargetABoxType = new JComboBox<Object>(methods.getAllFileTypes().keySet().toArray());
		...
		LinkedHashSet<String> hashSet = inputParamsMap.get(EXPRESSION_FILE);
